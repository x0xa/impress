...
class Application extends api.events.EventEmitter {
  ...
  //   callback <Function>
  //     err <Error> | <null>
  updateSchemas({ version }, callback) {
    if (this.latestSchema) {
      if (version === this.latestSchema.version) {
        callback(null);
        return;
      }
      if (version < this.latestSchema.version) {
        this.log.warning('Attempt to update schema to a lower version');
        callback(null);
        return;
      }
    }
  ...
  loadPlaceFile(placeName, path, file, callback) {
    ...
    impress.createScript(this, fileName, (err, exports) => {
      ...
      if (placeName === 'tasks') {
        ...
        const doneFileName = api.path.join(path, sectionName) + '.done';
        api.fs.writeFile(doneFileName, new Date().toISOString(), err => {
          if (err) this.log.error(impress.CANT_WRITE_FILE + doneFileName);
          callback();
        });
        return;
      }
